...
const tableRowHeaders = ["Name", "Country of Birth", "Age", "Gender"];
...
const Players = () => {
  ...
  const getData = () => {
    ...
    const tableData = players.map((player) => {
      if (
        (search.genderGroup === "All" ||
          player.gender.toLowerCase() === search.genderGroup.toLowerCase()) &&
  ...
  const handleSearchChange = (e) => {
    ...
    if (name === "name") {
      sortedAndFilteredData = categorizedData
        .filter((player) =>
        ...
        .filter((player) =>
          player.countryOfBirth
            .toLowerCase()
            .includes(search.countryOfBirth.toLowerCase())
        );
    }
    ...
    if (name === "countryOfBirth") {
      sortedAndFilteredData = categorizedData
        .filter((player) =>
          player.name.toLowerCase().includes(search.name.toLowerCase())
        )
        .filter((player) =>
          player.countryOfBirth.toLowerCase().includes(value.toLowerCase())
        );
    }
  ...
  return (
    <div className="container">
      <h3 className="accent-color" style={{ textAlign: "left" }}>
        Search through Players
      </h3>
      <div className="flex wrap justify-between">
        <div
          className="flex wrap"
          style={{ minWidth: "250px", maxWidth: "60%" }}
        >
          <TextField
            name="name"
            id="outlined-basic"
        ...
        <div className="flex align-start">
          {filterApplied() && (
            <Button
              ...
              height={70}
              startIcon={<ClearIcon />}
              sx={{ height: 40, margin: "0px !important" }}
          ...
          <Box sx={style} className="large-modal">
            <div className="flex-column">
              <div className="flex justify-between align-center">
                <h2 className="accent-color modal-title">
                  {currentPlayer?.firstName}{" "}
                  {currentPlayer?.middleName && `${currentPlayer?.middleName} `}
                  {currentPlayer?.familyName}
                </h2>
                <div className="close-icon">
                  <ClearIcon
                    className="pointer accent-color"
                    onClick={handleClose}
                  />
                </div>
              </div>
              <div className="flex-column info-section">
                <h3 className="accent-color section-title-medium">
                  Personal Information
                </h3>
                <div>Gender: {currentPlayer?.gender}</div>
                <div>
                  Nation Competing For:&nbsp;{currentPlayer?.countryOfBirth}
                </div>
                <div>
                  Date of Birth:&nbsp;
                  {moment(new Date(currentPlayer?.dateOfBirth)).format(
                    "D MMMM YYYY"
                  )}
                </div>
                <div>
                  Age:&nbsp;{getAge(new Date(currentPlayer?.dateOfBirth))}
                </div>
                {(currentPlayer?.gameInfo?.plays ||
                  currentPlayer?.gameInfo?.backhand) && (
                  <div>
                    Plays:&nbsp;
                    {currentPlayer?.gameInfo?.plays &&
                      `${currentPlayer?.gameInfo?.plays},\xa0`}
                    {currentPlayer?.gameInfo?.backhand}
                  </div>
                )}
                {currentPlayer?.about && (
                  <div className="about-section">{currentPlayer?.about}</div>
                )}
              </div>
              <div className="flex-column info-section">
                <h3 className="accent-color section-title-medium">
                ...
                {currentPlayer?.phoneNumber &&
                  (currentPlayer?.phoneNumberVisibility?.toLowerCase() ===
                    "public" ||
                    userData?.role === "clubRep") && (
